...
| FANOU Osé 
    Amis & Chat
        ...
        . Liste des utilisateurs inscrits, consultation de profils
        . Envoi/réception/gestion des invitations d'amitié (accepter/refuser)
        . Module chat : sidebar des conversations, recherche d'amis
        . Affichage temps réel des messages (sockets Node.js ou intervalle JS 3s)
        . Envoi de messages texte et images|

| KEMAVO Deo 
    Back Office & Finalisation

        . Page de connexion admin distincte
        . Rôles Modérateur (gestion articles/utilisateurs) et Administrateur (gestion complète + modérateurs/admins)
        . Dashboard avec statistiques
        Tests globaux, sécurisation du code, commentaires
        . README.md (description, identifiants test, membres, tâches) + document Word de soumission|

| SEGLA Charlène Mahougnon 
    Fil d'actualité & Profil
        
        . Page d'accueil : affichage des articles (avatar, nom/prénom, description, image)
        . Système de likes/dislikes avec persistance et changement de couleur d'icône
        . Commentaires en AJAX (sans rechargement)
        . Gestion du profil personnel : modification infos, photo de profil, mot de passe|

| PADONOU Yann Smith 
    Authentification & Structure de base

        . Mise en place de l'architecture du projet (assets, vues/clients, vues/back-office, api, index.html)
        . Inscription avec envoi d'email de confirmation (template HTML)
        . Connexion avec gestion de session via sessionStorage
        . Mot de passe oublié (envoi d'email HTML de réinitialisation)
        . Base de données MySQL (tables utilisateurs, structure de base)|

## 🔗 Lien du dépôt

> [https://github.com/yannsmithp-maker/reseau_social]
